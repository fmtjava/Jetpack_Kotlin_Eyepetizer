package com.fmt.kotlin.eyepetizer.discover.utils;
...
public class AutoPlayUtils {
     ...
     */
    public static void onScrollPlayVideo(RecyclerView recyclerView, int viewId, int jzvdId, int firstVisiblePosition, int lastVisiblePosition) {
        if (JZUtils.isWifiConnected(recyclerView.getContext())) {
            for (int i = 0; i <= lastVisiblePosition - firstVisiblePosition; i++) {
                View child = recyclerView.getChildAt(i);
                View view = child.findViewById(viewId);
                if (view instanceof ConstraintLayout) {
                    Jzvd player = view.findViewById(jzvdId);
